#!/usr/bin/env bash
# SoooDreamy — Linux "SDK simulation" for the iOS app.
#
...
#
# Usage:
#   scripts/typecheck/typecheck.sh            # app + widgets
#   scripts/typecheck/typecheck.sh app        # app target only
#   scripts/typecheck/typecheck.sh widgets    # widget extension only
...
prepare_sources() {
    ...
    done < <(find "$IOS/SoooDreamy" "$IOS/Shared" "$IOS/Widgets" -name '*.swift' -print0)
}
...
rc=0
case "$TARGET" in
    app)     typecheck_target SoooDreamy SoooDreamy Shared || rc=1 ;;
    widgets) typecheck_target SoooDreamyWidgets Widgets Shared || rc=1 ;;
    all)     typecheck_target SoooDreamy SoooDreamy Shared || rc=1
             typecheck_target SoooDreamyWidgets Widgets Shared || rc=1 ;;
    *) echo "unknown target: $TARGET (app|widgets|all)" >&2; exit 2 ;;
esac
exit $rc
